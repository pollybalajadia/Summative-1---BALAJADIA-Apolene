...
#### Convert the value in the dictionary to a string
#### Convert all characters to uppercase characters
#### Remove the following pieces of text: <P>, </P>, <B>, </B>, <I>, </I>
#### Remove all the unnecessary punctuation symbols denoted in the string below (((r"&$@[].,'#()-\"!?’_;:/")))
#### Replace punctuations with a space or " "

data_string = str(jsondata)
new_script1 = data_string.upper()
final = ""

for char in new_script:
    # Remove the following pieces of text: <P>, </P>, <B>, </B>, <I>, </I>
    if char == "<":
        final
    
    # Remove all the unnecessary punctuation symbols denoted in the string below (r"&$@[],'#()-\"!?’_;:/")
    if char == "&" or "$" or "@" or "[" or "]" or "," or "'" or "#" or "(" or ")" or "-" or "\" or """ or "!" or "?" or "’" or "_" or ";" or ":" or "/":
        final += ""
    else:
        final += char
...
for optimal_Z <= 344:
    ...
    optimal_R +=1
    optimal_C +=1
    if (500*optimal_F + 750*optimal_R + 300*optimal_C) > 12000:
        continue
    
    optimal_F += 1
    optimal_R +=1
    optimal_C +=1
    if (50*optimal_F + 45*optimal_R + 75*optimal_C) > 1500:
        continue
...
def Collatz(start_number):
    ...
    while final != 1
        if start_number % 2 == 0:
            final += start_number / 2
        if start_number % 2 != 0:
            final += start_number * 3 + 1
    print(final)

#output -> {"collatz_sequence":,"collatz_length":,"max_collatz":,"sequence_sum":}
